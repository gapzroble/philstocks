...
trait MaTrait
{
    ...
    protected function calculateMa($symbol, $date)
    {
        ...
        for (; isset($low40[$index]); $index++, $i++) {
            ...
            $this->exec(
                'INSERT IGNORE INTO ma (symbol, date, ema20, low40, high40, ma50, ema15, close, uptrend, cross_low, cross_high, vol50, vol_above_ave, vol_1m) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                array($symbol, $quotes[$index]['date'], $ema20[$index], $low40[$index], $high40[$index], $ma50[$index], $ema15[$index], $close[$index], $uptrend, $crossLow, $crossHigh, $vol50[$index], $volup, $vol1m)
            );
            $this->output->write('.');
        }
        if ($i) {
        ...
        unset($high40, $low40, $ema20, $quotes, $index, $i, $ema15, $ma50, $close, $vol50);
    }
}
